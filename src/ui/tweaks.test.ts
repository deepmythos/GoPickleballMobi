...
describe("1. bỏ tiêu đề + phụ đề mục điều kiện thô (DOM và từ điển)", () => {
  ...
  it("phần điều kiện thô vẫn CÒN: nút mở/đóng + 21 giá trị thô", () => {
    ...
    expect(stats.length, "phải còn đủ 21 ô thông số thô").toBe(21);
...
describe("2. hình minh hoạ sân nằm trong nội dung mở rộng của hàng chói nắng", () => {
  ...
  it("các hàng yếu tố khác không có hình, và app chỉ vẽ đúng 1 hình khi sheet đóng", () => {
    const root = render(state());
    expect(byClass(root, "court-diagram").length, "phải có đúng 1 hình khi sheet đóng").toBe(1);
